...
public class WelcomeWindow : EditorWindow
{
    ...
    private void DrawHeader()
    {
        if (GUILayout.Button(top, textureButton))
            Application.OpenURL("www.emaceart.com");
    }

    private void DrawBody()
    {
        GUILayout.Label("Thanks for checkin EpicCubeWorld Winter edition", headingText);
        GUILayout.Space(1f);
        GUILayout.Label("This pack perfect match with the EpicCubeWorld Summer edition!", commonText);
        GUILayout.Space(20f);

        if (GUILayout.Button(image1, textureButton))
            Application.OpenURL("https://assetstore.unity.com/packages/3d/environments/landscapes/epicmobile-3d-voxel-forest-and-atolls-208273");
